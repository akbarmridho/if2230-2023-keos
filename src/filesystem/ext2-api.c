...
int8_t separate_filename_extension(char **filename, uint8_t *len_name, char (*ext)[4])
{
  ...
  if (idx_found != -1)
  {
    int slash_dis = idx_found - last_slash;
    ...
    // filename is ..
    if (slash_dis == 2 && (*filename)[idx_found - 1] == '.')
    {
      (*ext)[0] = '\0';
      return 0;
    }
    (*filename)[idx_found] = '\0';
    ext_temp_ptr = *filename + idx_found + 1;
    int ext_len = *len_name - (idx_found + 1);
